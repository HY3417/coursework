...
*Defining load
*/
int q = -5000; // Constant flux at bottom edge of the beam
...
ofstream myfile3 ("disp3.vtk");
if (myfile3.is_open()){
    myfile3 << "# vtk DataFile Version 4.0\n";
    myfile3 << "vtk output\n";
    myfile3 << "ASCII\n";
    myfile3 << "DATASET UNSTRUCTURED_GRID\n";
    myfile3 << "POINTS " << nnode << " double\n";
    for (int i = 0; i < 2; i++){
        for (int j = 0; j < nDof; j++){
            myfile3 << fixed <<setprecision(3)<< vCoord[i][j] << " ";
        }
        myfile3  << "0.0 ";
    }
    myfile3 << "\n";
    myfile3 << "CELLS " << nelem << " "<< (nnode_elem+1)*nelem << "\n";


    for (int i = 0; i < nelem; i++) {
        for (int j = 0; j < nnode_elem + 1; j++) {
            myfile3 << ElemNode1[i][j] << " ";
        }
        myfile3 << "\n";
    }
    myfile3 << "CELL_TYPES " << nelem <<"\n";
    for (int i = 0; i < nelem; i++) {
        myfile3 << "9\n";
    }

    myfile3 << "POINT_DATA " << nDof <<"\n";
    myfile3 << "FIELD FieldData 3" << "\n";
    myfile3 << "disp 3 " << nDof << " double\n";
    for (int i = 0; i < nDof; i++) {
        myfile3 << fixed << setprecision(5) << T[i] <<" ";
    }
    }
    myfile3.close();
return 0;
