...
ofstream myfile1 ("disp1.vtk");
if (myfile1.is_open()){
    myfile1 << "# vtk DataFile Version 4.0\n";
    myfile1 << "vtk output\n";
    myfile1 << "ASCII\n";
    myfile1 << "DATASET UNSTRUCTURED_GRID\n";
    myfile1 << "POINTS " << nnode << " double\n";
    for (int i = 0; i < 2; i++){
        for (int j = 0; j < nDof; j++){
    ...
    myfile1 << "\n";
    myfile1 << "CELLS " << nelem << " "<< (nnode_elem+1)*nelem << "\n";


    for (int i = 0; i < nelem; i++) {
        for (int j = 0; j < nnode_elem + 1; j++) {
            myfile1 << ElemNode1[i][j] << " ";
        }
        myfile1 << "\n";
    }
    myfile1 << "CELL_TYPES " << nelem <<"\n";
    for (int i = 0; i < nelem; i++) {
        myfile1 << "9\n";
    }

    myfile1 << "POINT_DATA " << nDof <<"\n";
    myfile1 << "FIELD FieldData 1" << "\n";
    myfile1 << "disp 1 " << nDof << " double\n";
    for (int i = 0; i < nDof; i++) {
        myfile1 << fixed << setprecision(5) << T[i] <<" ";
    }
    }
    myfile1.close();
return 0;
